...
fn stereo_independent_channels<'a>(left: &'a [i32], right: &'a [i32]) -> [EncodedChannel<'a>; 2] {
    [
        EncodedChannel {
            bits_per_sample: ENCODE_BITS_PER_SAMPLE,
            samples: left,
...
fn left_side_channels<'a>(left: &'a [i32], side: &'a [i32]) -> [EncodedChannel<'a>; 2] {
    [
        EncodedChannel {
            bits_per_sample: ENCODE_BITS_PER_SAMPLE,
            samples: left,
...
fn right_side_channels<'a>(side: &'a [i32], right: &'a [i32]) -> [EncodedChannel<'a>; 2] {
    [
        EncodedChannel {
            bits_per_sample: ENCODE_BITS_PER_SAMPLE + 1,
            samples: side,
...
fn mid_side_channels<'a>(mid: &'a [i32], side: &'a [i32]) -> [EncodedChannel<'a>; 2] {
    [
        EncodedChannel {
            bits_per_sample: ENCODE_BITS_PER_SAMPLE,
            samples: mid,
...
fn stereo_side_samples(left: &[i32], right: &[i32]) -> Vec<i32> {
    left.iter()
        .zip(right)
...
fn stereo_mid_samples(left: &[i32], right: &[i32]) -> Vec<i32> {
    left.iter()
        .zip(right)
